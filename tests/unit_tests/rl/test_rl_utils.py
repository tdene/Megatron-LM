...
class TestRLUtils:
    ...
    def test_prep_wandb_metrics(self, initialize_model_parallel, inject_placeholders):
        ...
        # avg-token policy lag is token-weighted over the (epoch, token_count) segments:
        # rollout 0 = 6-(4*1+5*2)/3, rollout 1 = 6-(2*2+3*1)/3, rollout 2 = 6-5,
        # rollout 3 = 6-(0*3+1*1)/4 = [4/3, 11/3, 1, 5.75]
        assert np.isclose(metrics["staleness/policy/avg/mean"], np.mean([4 / 3, 11 / 3, 1, 5.75]))
        ...
        # avg-token kv lag: rollout 0 = 6-(4*2+5*1)/3, rollout 1 = 6-(3*1+4*2)/3,
        # rollout 2 = 6-5, rollout 3 = 6-(1*2+2*2)/4 = [5/3, 7/3, 1, 4.5]
        assert np.isclose(metrics["staleness/kv_cache/avg/mean"], np.mean([5 / 3, 7 / 3, 1, 4.5]))
