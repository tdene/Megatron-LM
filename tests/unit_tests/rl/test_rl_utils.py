...
from megatron.rl.rollout_granularity import resolve_rl_generation_lag
...
def make_token_rollout(trajectory, logprobs, generation_mask=None, reward=1.0, problem_id="p"):
    """TokenRollout fixture; staleness metadata now rides the request ledger, not the rollout."""
    return TokenRollout(
...
class TestRLUtils:
    ...
    def test_resolve_rl_generation_lag(
        ...
        # The RolloutPipeline gate rounds the implied G-submission slot count and
        # clamps it to at least one unit: max(1, round((lag + 1) * P)).
        assert (
            max(1, round((args.rl_generation_lag + 1) * args.grpo_prompts_per_step))
            == expected_tasks
        )
    ...
    def test_prepare_data_for_update(self, initialize_model_parallel):
        ...
        # recurs once per dp group, and mt1's turn 2 and mt2's turn 3 share one
        # token stream, so that bucket holds two records per dp group.
        mt_ledger = {
            _token_stream_key([1], [2, 3]): [_ledger_record(0) for _ in range(dp)],
            _token_stream_key([1, 2, 3], [4]): [_ledger_record(0) for _ in range(2 * dp)],
    ...
    def test_prep_wandb_metrics_empty_wave(self):
    ...
    def test_prep_wandb_metrics_skips_unjoined_rollouts(self):
        # Rollouts that never joined the ledger (text rollouts, zero-turn failure
        # placeholders) carry None epoch summaries; staleness telemetry and the
        # row-aligned rollout table keep only the joined rollouts.
        writer = MagicMock()
        metrics = rl_utils.prep_wandb_metrics(
            writer,
            traj_lens=[[3, 7]],
            ...
            num_turns=[[1, 1]],
            advantages=[0.5, -0.5],
            policy_first_epoch=[[4, None]],
            policy_avg_epoch=[[4.5, None]],
            policy_last_epoch=[[5, None]],
            kv_first_epoch=[[4, None]],
            kv_avg_epoch=[[4.5, None]],
            kv_last_epoch=[[5, None]],
            completed_epochs=[[5]],
            num_evictions=[[2, 0]],
            env_ids=[['MEGAENV', 'MEGAENV']],
            problem_ids=[['a', 'b']],
            current_iteration=6,
        )
        # Only the joined rollout feeds staleness: first lag 6-4, avg 6-4.5, last 6-5.
        assert metrics["staleness/policy/first/mean"] == 2
        assert metrics["staleness/policy/avg/mean"] == 1.5
        assert metrics["staleness/policy/last/min"] == 1
        # Eviction totals still count every rollout (unjoined ones hold zero).
        assert metrics["total_eviction_count"] == 2
        assert metrics["max_num_evictions"] == 2
        # The rollout table is row-aligned over joined rollouts only.
        table_call = next(
            c for c in writer.Table.call_args_list if "traj_length" in c.kwargs.get("columns", [])
        )
        assert table_call.kwargs["data"] == [
            ("MEGAENV", "a", 1.0, 3, 2, 2, 1.5, 1, 2, 1.5, 1)
        ]

        # A window where nothing joined emits no staleness distributions at all.
        writer = MagicMock()
        metrics = rl_utils.prep_wandb_metrics(
            writer,
            traj_lens=[[3]],
            ...
            num_turns=[[1]],
            advantages=[0.5],
            policy_first_epoch=[[None]],
            policy_avg_epoch=[[None]],
            policy_last_epoch=[[None]],
            kv_first_epoch=[[None]],
            kv_avg_epoch=[[None]],
            kv_last_epoch=[[None]],
            completed_epochs=[[]],
        ...
        assert "staleness/policy/first/mean" not in metrics
        table_call = next(
            c for c in writer.Table.call_args_list if "traj_length" in c.kwargs.get("columns", [])
        )
        assert table_call.kwargs["data"] == []
        ...
        for turns, masks, match in (
            ...
            with pytest.raises(AssertionError, match=match):
                rl_utils.compute_group_stats(
                    [RolloutGroup(rollouts=[token_rollout(turns, masks)])],
                    MockTokenizer(),
                    8,
                    {},
                )
...
def _ledger_record(epoch, num_evictions=0):
    ...
    def test_compute_group_stats_excludes_placeholders_from_metric_fields(self):
        ...
        # All-generated masks: the whole turn keys as generation with an empty prompt.
        # Placeholders have no turns, so they need (and consume) no records.
        ledger = {
        ...
        # Per-rollout lists keep the placeholder entries (as None summaries):
        # alignment with rewards and num_turns is what lets prep_wandb_metrics
        # mask them downstream.
        assert stats.num_turns == [[1, 1, 0], [0, 0, 0]]
        assert stats.policy_first_epoch == [[5, 6, None], [None, None, None]]
        assert stats.policy_last_epoch == [[5, 6, None], [None, None, None]]
        assert stats.traj_lens == [[3, 4, 0], [0, 0, 0]]
        # Per-turn lists exclude placeholders entirely: no sentinel epoch stamp
        ...
        # End to end: placeholders never reach the staleness metrics.
        metrics = rl_utils.prep_wandb_metrics(
            MagicMock(),
            stats.traj_lens,
            stats.turn_lens,
        ...
        assert metrics["staleness/policy/first/max"] == 2  # 7 - 5, no sentinel rows
    ...
    def test_prep_wandb_metrics_all_placeholder_wave_keeps_failure_counters(self):
        # Rewards exist (the zero placeholders shape the GRPO baseline) but every
        # rollout is a zero-turn placeholder: the wave skips the aggregates and
        # still reports its failure counters.
        metrics = rl_utils.prep_wandb_metrics(
            MagicMock(),
            traj_lens=[[0, 0]],
            turn_lens=[[]],
            ...
            num_turns=[[0, 0]],
            advantages=[0.0, 0.0],
            policy_first_epoch=[[None, None]],
            policy_avg_epoch=[[None, None]],
            policy_last_epoch=[[None, None]],
            kv_first_epoch=[[None, None]],
            kv_avg_epoch=[[None, None]],
            kv_last_epoch=[[None, None]],
            completed_epochs=[[]],
            num_evictions=[[0, 0]],
            env_ids=[["swe", "swe"]],
            problem_ids=[["a", "b"]],
