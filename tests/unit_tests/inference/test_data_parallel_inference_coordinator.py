...
async def graceful_shutdown(engine, client=None, *, timeout=10.0):
    ...
    PAUSED (skips straight to STOP).  Sending a duplicate PAUSE to an
    already-paused engine would trip asserts in both the engine and the
    coordinator, so this helper checks engine.state first.
...
class TestCoordinator:
    ...
    async def test_control_logic_lifecycle(self, initialize_model_parallel):
        ...
        Lifecycle exercised:
          RUNNING -> requests -> PAUSE -> requests-during-pause -> UNPAUSE ->
          requests -> PAUSE -> SUSPEND -> RESUME -> UNPAUSE -> requests ->
          PAUSE -> STOP
        ...
        try:
            if torch.distributed.get_rank() == 0:
                ...
                # ── RESUME ───────────────────────────────────────────
                # paused was already set and never cleared, so clear it
                # ourselves so we can wait for the RESUMING->PAUSED re-set.
                engine.paused.clear()
        ...
        # STOP, SUSPEND, and RESUME all require PAUSED (or SUSPENDED for
        # RESUME).  Sending any of them from RUNNING should crash the
        # engine with AssertionError.
        #
        # Note: DummyEngine overrides run_engine_with_coordinator to
        # bypass @trace_async_exceptions, so the raw AssertionError
        # propagates instead of being converted to sys.exit(1).
        for signal_name, signal_fn in [
            ...
            try:
                ...
                with pytest.raises(AssertionError, match="Received .+ in state"):
