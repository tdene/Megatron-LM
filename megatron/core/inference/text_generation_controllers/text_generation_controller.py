...
class TextGenerationController:
    ...
    def _init_dynamic_sampling_tensors(self):
        ...
        # Need a sentinel slot for `nonzero_static` (per_request_logprobs). When the
        # flashinfer sampling backend's `sample_kernel` runs, this assignment is
        # later rebound to sample_kernel's output (a CudaGraphManager-managed static
        # buffer). The pre-allocation here is the safe initial value for paths that
        # read `_sampled_tokens_cuda` before the first sample_kernel call.
    ...
    def _dynamic_step_sample_logits_and_verify_tokens(self, input_ids: Tensor):
        ...
        # graph_last_logits: `speculative_required_logit_indices()` returns a slice of
        # the static `active_logit_idxs` buffer sized to `num_last_token_logits`, which
        # is padded under cuda graph. flashinfer's old per-step F.pad is no longer needed.
    ...
    async def async_generate_output_tokens_dynamic_batch(
        ...
        with torch.inference_mode():
            # Sample bookkeeping is now internal to the Sampling backend's `sample_kernel`
            # (flashinfer); inline torch bucketization (`_dynamic_step_sample_bookkeeping`) is
            # gone. Log-prob bookkeeping was already issued earlier on
            # `_pre_forward_bookkeeping_stream` (per_request_logprobs) and consumed via the
            # pinned tensors `_log_prob_count_pinned` / `_top_n_max_pinned`.
