...
class LogProbsPrefill:
    ...
    def indexing(self, context, *, eager: bool = False) -> None:
        ...
        padded_token_count = context.padded_active_token_count
        key = ("prefill_idx", padded_request_count, padded_token_count)
        for _ in self._graphs(
            key, skip_warmup=True, eager=eager, pool=CudaGraphManager.global_mempool
        ):
            self._outputs[key] = self.indexing_kernel(context)
    ...
    def calculate(
        ...
        *,
        ...
        ri, cu_ml, li, li_range, mt = self._outputs[
            ("prefill_idx", padded_request_count, padded_token_count)
        ]

        key = ("prefill_sm", padded_request_count, padded_token_count)
        for _ in self._graphs(
            key, skip_warmup=True, eager=eager, pool=CudaGraphManager.global_mempool
        ):
            self._outputs[key] = (
                self.softmax_kernel(logits, new_tokens, ri, cu_ml, li, li_range, mt),
            )

        (slp,) = self._outputs[key]
        active_request_count = context.total_request_count - context.paused_request_count
