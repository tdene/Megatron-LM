...
class LogProbsDecode:
    ...
    def calculate(
        ...
        *,
        ...
        """
        padded_count = context.padded_active_request_count
        ri, padded_arange = self._outputs[("decode_idx", padded_count)]

        key = ("decode_sm", padded_count)
        for _ in self._graphs(
            key, skip_warmup=True, eager=eager, pool=CudaGraphManager.global_mempool
        ):
            self._outputs[key] = self.softmax_kernel(logits, new_tokens, ri, padded_arange)

        slp, sl = self._outputs[key]
        active_request_count = context.total_request_count - context.paused_request_count
