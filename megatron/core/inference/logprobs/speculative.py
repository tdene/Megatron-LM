...
class LogProbsSpeculative:
    ...
    def prefill_indexing(self, context, *, eager: bool = False) -> None:
        ...
        key = ("spec_fp_idx", padded_request_count, padded_token_count)
        for _ in self._graphs(
            key, skip_warmup=True, eager=eager, pool=CudaGraphManager.global_mempool
        ):
            self._outputs[key] = self.prefill_indexing_kernel(context)
    ...
    def softmax(self, context, logits: Tensor, *, eager: bool = False) -> None:
        ...
        padded_decode_count = context.padded_batch_dimensions.decode_req_count
        padded_prefill_count = context.padded_batch_dimensions.prefill_req_count
        key = ("spec_sm", padded_decode_count, padded_prefill_count)
        for _ in self._graphs(
            key, skip_warmup=True, eager=eager, pool=CudaGraphManager.global_mempool
        ):
            self._outputs[key] = self.softmax_kernel(context, logits)
    ...
    def calculate(
        ...
        *,
        ...
        padded_decode_count = context.padded_batch_dimensions.decode_req_count
        padded_prefill_count = context.padded_batch_dimensions.prefill_req_count
        decode_log_probs, prefill_log_probs = self._outputs[
            ("spec_sm", padded_decode_count, padded_prefill_count)
        ]

        key = ("spec_ga", padded_decode_count, padded_prefill_count)
        for _ in self._graphs(
            key, skip_warmup=True, eager=eager, pool=CudaGraphManager.global_mempool
        ):
            self._outputs[key] = self.gather_kernel(
                context,
                decode_log_probs,
                prefill_log_probs,
                new_tokens,
        ...
        decode_gathered, prefill_gathered = self._outputs[key]
        ...
        if not only_last and num_prefill > 0:
            ...
            fp_ri, fp_cu_ml, fp_li, li_range, mt, fp_count_gpu = self._outputs[
                ("spec_fp_idx", padded_request_count, padded_token_count)
            ]

            fp_key = ("spec_fp_sm", padded_request_count, padded_token_count)
            for _ in self._graphs(
                fp_key, skip_warmup=True, eager=eager, pool=CudaGraphManager.global_mempool
            ):
                self._outputs[fp_key] = (
                    LogProbsPrefill.softmax_kernel(
                        logits, new_tokens, fp_ri, fp_cu_ml, fp_li, li_range, mt
                    ),
                )

            (fp_slp,) = self._outputs[fp_key]
