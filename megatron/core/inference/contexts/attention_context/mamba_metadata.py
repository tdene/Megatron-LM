...
class MambaMetadata:
    ...
    def update(
        ...
        with GPU scalars and pre-allocated index tensors for the real/padding
        boundary.
        ...
        Args:
            ...
            intermediate_offsets_gpu (Tensor): `[max_requests, 3]` int32 GPU buffer of
                per-request intermediate token offsets, or None when no slot allocator is attached.
            intermediate_counts_gpu (Tensor): `[max_requests]` int32 GPU buffer of
        ...
        if padded_prefill_count > 0:
            ...
            # Intermediate state extraction metadata.
    ...
    def _update_intermediate_metadata(
        ...
        Uses GPU scalar offsets and fixed-shape ops. When no slot allocator
        exists (buffers are None), fills with safe defaults unconditionally.
        ...
        if intermediate_offsets_gpu is not None and padded_prefill_count > 0:
            ...
            cum_chunks = self._cum_chunks_buffer[: padded_prefill_count + 1].to(torch.int64)
            cum_chunks[0] = 0
