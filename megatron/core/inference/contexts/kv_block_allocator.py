...
class KVBlockAllocator:
    ...
    def __init__(
        ...
        # When ``max_kv_block_count`` is provided the buffer is pre-sized to
        # ``total_count + (total_count - 1) * max_kv_block_count`` so that
        # ``pad_for_release`` never needs to reallocate it.  Avoiding the
        # reallocation is important: allocating then freeing a large tensor
        # between CUDA graph captures shifts the caching allocator's free-block
        # topology and can cause captured graphs to replay at wrong addresses.
        device = torch.cuda.current_device()
        max_requests_bound = total_count - 1  # upper bound on max_requests
        init_pad = max_requests_bound * max_kv_block_count if max_kv_block_count > 0 else 0
        self.block_bag = torch.empty(
            total_count + init_pad, dtype=torch.int32, device=device
        )
        self.block_bag[:total_count] = torch.arange(total_count, dtype=torch.int32, device=device)
        if init_pad > 0:
            self.block_bag[total_count:] = self.dummy_block_idx

        # GPU mirror of the stack pointer, used by the graphed update_requests
        # body to compute resume counts without a CPU sync.
        self.total_avail_gpu = torch.tensor(
            [self.total_avail], dtype=torch.int32, device=torch.cuda.current_device()
        )

        # Set by ``pad_for_release`` once the context knows max_requests.
        # Until then, the graphed-release and graphed-allocate helpers
        # cannot be used.
        self.max_requests: int = 0
        self.max_release_per_step: int = 0
        self._release_arange_i32: Optional[Tensor] = None
        self._release_arange_i64: Optional[Tensor] = None
        self._alloc_arange: Optional[Tensor] = None
        ...
        if self.enable_prefix_caching:
            ...
            # pairs here for any block whose ref count just dropped to zero
            # and needs host-side ``kv_hash_to_block_id`` + callback cleanup.
            # Drained once per ``update_requests`` call via ``drain_pending_dereg``
            # (called from the post-phase sync in ``update_requests``).
            # Size is deferred to ``pad_for_release`` because it depends on
            # ``max_release_per_step``.
            self._pc_pending_dereg_ids: Optional[Tensor] = None
            self._pc_pending_dereg_hashes: Optional[Tensor] = None
            self._pc_pending_dereg_count: Optional[Tensor] = None
            # Host-side flag: True when a prefix-aware release has run
            # since the last drain. Lets ``drain_pending_dereg`` skip the
            # GPU sync when no events could have been enqueued.
            self._pc_dereg_may_have_events: bool = False
            # Per-block workspace for the prefix-aware graphed release.
            # Allocated by ``pad_for_release`` alongside the dereg queue.
            self._pc_released_bitmap: Optional[Tensor] = None
            self._pc_block_arange: Optional[Tensor] = None

            if self.prefix_caching_eviction_policy == PrefixCachingEvictionPolicy.LRU:
                # GPU-resident monotonic LRU clock.  Bumped once per engine
                # step via ``bump_lru_clock`` and independently by the graphed
                # allocate path for staleness detection.
                self.lru_clock_gpu = torch.zeros(
                    (), dtype=torch.int64, device=torch.cuda.current_device()
                )

                # Append-only sorted list of cached block IDs. Because
                # ``lru_clock_gpu`` increments per allocation, each block's
                # ``block_timestamps`` is unique; the list is sorted by
                # that timestamp and insertion order matches. Appends
                # never re-sort.
                #
                # A parallel array ``_lru_cached_list_ts`` records the
                # block's timestamp at append time. At query time, an
                # entry is "valid" iff ``recorded_ts == current_ts`` for
                # that block — a mismatch means the block was allocated
                # and possibly re-cached, invalidating this entry (a new
                # entry for the fresh cache exists at a later position).
                #
                # Compaction runs periodically (every
                # ``_lru_compaction_interval`` calls), driven by a
                # host-side counter rather than by syncing the GPU
                # length. The interval is picked so the list cannot
                # overflow between compactions in the worst case.
                # Pre-allocated scratch buffers
                # ``_lru_compact_scratch{,_ts}`` hold the pre-compaction
                # snapshot so compaction avoids allocation.
                # Size is deferred to ``pad_for_release``.
                self._lru_cached_list: Optional[Tensor] = None
                self._lru_cached_list_ts: Optional[Tensor] = None
                self._lru_cached_list_len: Optional[Tensor] = None
                self._lru_compact_scratch: Optional[Tensor] = None
                self._lru_compact_scratch_ts: Optional[Tensor] = None
                self._lru_calls_since_compact: int = 0
                self._lru_compaction_interval: int = 0
    ...
    def reset(self) -> None:
        ...
        # for UVM performance. Fill in place to preserve the tensor's memory
        # address -- captured CUDA graphs reference block_bag at a specific
        # address, and re-allocating it would invalidate those captures.
        ...
        self.block_bag[: self.total_count].copy_(
            torch.arange(self.total_count, dtype=torch.int32, device=device)
        )
        if self.max_release_per_step > 0:
            # Fill the release scratch region with dummy_block_idx so stale
            # data can never leak back out of an over-length write.
        ...
        if self.enable_prefix_caching:
            ...
            # Reset graphed-path scaffolding state (only if pad_for_release
            # has been called — before that, these buffers don't exist).
            if self._pc_pending_dereg_count is not None:
            ...
            if (
                ...
                and self._lru_cached_list is not None
            ):
    ...
    # =========================================================================
    # Graphed-release support
    # =========================================================================

    def pad_for_release(self, max_requests: int, max_kv_block_count: int) -> None:
        """Extend the block bag with scratch space for shape-stable releases.

        The graphed release path writes ``max_requests * max_kv_block_count``
        entries at the current stack pointer and advances the pointer by the
        number of valid entries, leaving the over-written dummy values above
        the new stack pointer. That over-write has to land inside
        ``block_bag``, so we extend it once at setup time.

        Also allocates the static aranges used by ``release_memory_blocks_gpu``
        and ``allocate_memory_blocks_gpu``.

        Idempotent. Must be called before any graphed release or allocate.

        Args:
            max_requests: Maximum number of concurrent requests.
            max_kv_block_count: Maximum number of KV blocks per request.
        """
        max_release_per_step = max_requests * max_kv_block_count
        if (
            self.max_release_per_step == max_release_per_step
            and self._release_arange_i32 is not None
        ):
            return
        ...
        # Extend block_bag if the pre-sized buffer from __init__ is too small.
        # When max_kv_block_count was passed to __init__, the buffer is already
        # large enough and this branch is skipped — avoiding a reallocation that
        # would shift the caching allocator's free-block topology.
        needed = self.total_count + max_release_per_step
        if self.block_bag.numel() < needed:
            old_bag = self.block_bag
            new_bag = torch.empty(needed, dtype=torch.int32, device=device)
            new_bag[: self.total_count] = old_bag[: self.total_count]
            new_bag[self.total_count :] = self.dummy_block_idx
            self.block_bag = new_bag

        # Static aranges used as fixed-size index sources.
        # _release_arange_* is (max_release_per_step,) and covers the maximum
        # write footprint on the release path.  _alloc_arange* is
        # (max_requests,) and is the fixed-size read range for the graphed
        # allocate path.
        self._release_arange_i32 = torch.arange(
            max_release_per_step, dtype=torch.int32, device=device
        )
        self._release_arange_i64 = torch.arange(
            max_release_per_step, dtype=torch.int64, device=device
        )
        ...
        # ── Prefix-caching graphed-path buffers ──
        if self.enable_prefix_caching:
            # Dereg queue: sized to hold events from both graphs running
            # back-to-back without an intermediate drain.  Graph 1 can
            # enqueue up to max_release_per_step (classify release) +
            # max_requests (resume_1 LRU eviction).  Graph 2 can enqueue
            # up to max_release_per_step (eviction release) + max_requests
            # (resume_2 LRU eviction).  Plus 1 sink slot.
            dereg_capacity = 2 * max_release_per_step + 2 * max_requests + 1
            ...
            # Per-block workspace used by the prefix-aware graphed release
            # to deduplicate released block IDs (prefix caching can share a
            # block across multiple requests, so the same block may appear
            # several times in a single release's packed buffer).
            self._pc_released_bitmap = torch.zeros(
                (self.total_count,), dtype=torch.bool, device=device
            )
            # Static source for scatter-packs that iterate over the full
            # block pool (e.g., dereg queue append uses block IDs 0..N-1).
            self._pc_block_arange = torch.arange(self.total_count, dtype=torch.int32, device=device)

            if self.prefix_caching_eviction_policy == PrefixCachingEvictionPolicy.LRU:
                # ``_lru_cached_list`` is sized to ``2 * total_count`` so it
                # can absorb up to one full generation of stale entries
                # before compaction. A final +1 slot is the scatter-pack
                # sink. Overflow is prevented by calling
                # ``lru_maybe_compact`` every ``_lru_compaction_interval``
                # update_requests calls, a host-driven periodic trigger
                # that avoids a GPU sync for the length check.
                self._lru_cached_list = torch.full(
                    (2 * self.total_count + 1,), -1, dtype=torch.int32, device=device
                )
                # Parallel array: recorded timestamp for each list entry.
                # Used by the staleness check to distinguish still-cached
                # blocks from blocks that were re-allocated after being
                # appended.
                ...
                # Pre-allocated snapshot buffers used by compaction in
                # place of a ``.clone()``. Keeps the compaction path
                # allocation-free so it's cheap even if it runs often.
                ...
                # Compaction interval. The list capacity is
                # ``2 * total_count`` (plus a sink slot). At any moment
                # it holds at most ``total_count`` *valid* entries (one
                # per block) plus any *stale* entries accumulated since
                # the last compaction. Compaction removes stale entries
                # but cannot shrink the valid set, so the effective
                # budget for stale growth between compactions is only
                # ``total_count`` slots.
                #
                # Worst-case stale appends per ``update_requests`` call
                # are bounded by ``2 * max_release_per_step`` — the
                # classify release and the evict release can each
                # re-cache up to one full release's worth of blocks.
                #
                # Leave ~25% headroom: ``interval <= 0.75 * total_count
                # / worst_per_call``.
                worst_per_call = max(1, 2 * max_release_per_step)
    ...
    def drain_pending_dereg(self) -> None:
        ...
        """
        if not self.enable_prefix_caching or self._pc_pending_dereg_count is None:
            return
        ...
        ids = self._pc_pending_dereg_ids[:count].tolist()
        hashes = self._pc_pending_dereg_hashes[:count].tolist()
    ...
    def allocate_memory_blocks_gpu(self, count_gpu: Tensor) -> Tensor:
        ...
        """
        assert (
            self._alloc_arange is not None
        ), "pad_for_release() must be called before allocate_memory_blocks_gpu()"
    ...
    def release_memory_blocks_gpu(self, packed_blocks: Tensor, num_valid_gpu: Tensor) -> None:
        ...
        """
        assert (
            self._release_arange_i32 is not None
        ), "pad_for_release() must be called before release_memory_blocks_gpu()"
    ...
    def _push_masked_to_pool(self, push_mask: Tensor) -> None:
        ...
        ``pad_for_release``, so the sink at index
    ...
    def release_memory_blocks_prefix_aware_gpu(
        ...
        assert (
            self._release_arange_i32 is not None
        ), "pad_for_release() must be called before release_memory_blocks_prefix_aware_gpu()"
        assert packed_blocks.numel() == self.max_release_per_step
    ...
    def allocate_memory_blocks_prefix_aware_gpu(self, count_gpu: Tensor) -> Tensor:
        ...
        assert (
            self._alloc_arange is not None
        ), "pad_for_release() must be called before allocate_memory_blocks_prefix_aware_gpu()"
    ...
    def lru_maybe_compact(self) -> None:
        ...
        interval was sized in :meth:`pad_for_release` so the list
        ...
        if (
            ...
            or self._lru_cached_list is None
        ):
            return
