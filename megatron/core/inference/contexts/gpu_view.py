...
class ContextGPUView:
    ...
    def __init__(
        ...
        # (active slots) plus `_pad_cpu_active_slices` (trailing sentinel).
        # The trailing slot at index `max_requests` is the sentinel target for
        # the log-prob kernels' `nonzero_static(..., fill_value=max_requests)`
        # lookup; the unified H2D propagates the CPU-set sentinel each step.
