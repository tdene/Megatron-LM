...
class DynamicInferenceContext(BaseInferenceContext):
    ...
    def pad_active_slices(self):
        ...
        # Some tensors need to be padded at the request level.
        ...
        if self.config.sampling_backend == 'flashinfer':
            if self.padded_active_request_count > active_request_count:
                ...
                if not self.config.materialize_only_last_token_logits:
                    self.active_request_last_token_idxs[active_request_count:self.padded_active_request_count].fill_(0)
