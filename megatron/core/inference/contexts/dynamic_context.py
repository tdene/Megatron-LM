...
class DynamicInferenceContext(BaseInferenceContext):
    ...
    def initialize_all_tensors(self) -> None:
        ...
        # Sized ``max_requests + 1``: per_request_logprobs's prefill kernel uses
        # ``nonzero_static(..., fill_value=max_requests)`` and gathers from this
        # tensor, so the trailing slot at index ``max_requests`` is a permanent
        # zero sentinel (the rest of the bookkeeping never writes there).
        ...
        # The trailing slot at index ``max_requests`` is the permanent sentinel
        # target for the per_request_logprobs prefill kernel's
        # ``nonzero_static(..., fill_value=max_requests)`` lookup; it stays at
        # 0 throughout (the GPU cumsum only writes ``[:n]``).
        self.active_request_last_token_idxs = torch.zeros(
            (self.max_requests + 1,), dtype=torch.int32, device=gpu_device
        )
        ...
        # GPU scalars for freely-varying counts. Used by graphable ops via
        # ``torch.where`` so slice shapes stay fixed across CUDA graph captures.
        # Per-step values are staged into the matching pinned-CPU mirror
        # ``_context_op_metadata_cpu`` by ``prepare_attn_init`` (six int writes
        # against pinned memory) and copied to GPU as a single coalesced H2D
        # captured inside ``run_attn_init_graph_body``. Replaces what used to
        # be six per-step ``tensor.fill_(int)`` kernel launches.
        self._context_op_metadata_gpu = torch.zeros(
            6, dtype=torch.int32, device=gpu_device
        )
        self._context_op_metadata_cpu = torch.zeros(
            6, dtype=torch.int32, device='cpu'
        ).pin_memory()
    ...
    def prepare_attn_init(
        ...
        *,
        ...
        # Stage the GPU scalars that gate ``torch.where`` mask boundaries into
        # the pinned-CPU mirror. The single coalesced H2D into
        # ``_context_op_metadata_gpu`` is performed inside
        # ``run_attn_init_graph_body`` so it is captured by the per-bucket
        # CUDA graph; replays then pick up whatever the CPU mirror holds at
        # replay time.
    ...
    def run_attn_init_graph_body(self, eager: bool = False, cache_key=None) -> tuple:
        ...
        # Single coalesced H2D for the six per-step GPU scalars. Captured in
        # the graph so replays always copy whatever ``prepare_attn_init`` just
        # staged into the pinned CPU mirror.
