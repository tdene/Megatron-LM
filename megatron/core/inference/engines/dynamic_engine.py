...
class DynamicInferenceEngine(AbstractEngine):
    ...
    def reset(self) -> None:
        ...
        # lag < 0 → unbounded (asyncio maxsize=0), lag == 0 → maxsize=1 (synchronous
        # via join() after each put), lag > 0 → maxsize=lag.
        queue_maxsize = 0 if self.request_bookkeeping_lag < 0 else max(self.request_bookkeeping_lag, 1)
