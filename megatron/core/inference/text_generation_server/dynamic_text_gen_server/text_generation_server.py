...
async def _run_text_gen_server(
    ...
    try:
        ...
        app.config['shared_requests'] = shared_requests
...
def start_text_gen_server(
    ...
    # Shared dict for the responses endpoint's POST/GET polling pattern.
    # Each worker process can read/write this dict so GET polls can be
    # served by any replica, not just the one that handled the POST.
    _MANAGER = mp.Manager()
    shared_requests = _MANAGER.dict()
...
def stop_text_gen_server():
    ...
    # Clean up the multiprocessing Manager
