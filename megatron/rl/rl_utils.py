...
def get_rollout_generator(args, inference_interface, n_prompts, samples_per_group):
    ...
    if _ROLLOUT_GENERATOR is None:
        # Fresh pipeline: clear the in-flight counter. The pipeline persists
        # across iterations (and so does its in-flight count), so this only
        # fires on creation.
        reset_inflight()
...
def colocated_inference(
    ...
    Returns:
        (rollouts, request_ledger): collected rollout groups on rank 0 (placeholder
        data on other ranks) and the drained per-request metadata ledger (gathered
        across ranks, identical everywhere).
    """
    args = get_args()
    ...
    with nvtx_range("rl/rollout-collection", time=True):
        ...
        # Drain the engines' finished-request ledgers (collective: every rank ends
        # up with the same gathered union). Engines are suspended at this point.
        with nvtx_range("rl/drain-request-ledger", time=True):
...
def get_environment_rollouts(
    ...
    if run_inference:
        ...
        # Banked groups are consumed into the training batch here: they leave
...
def single_turn_termination_ok(
    ...
    Legitimate ends: truncation at the context window, an eod token, or —
    when the agent stamped its per-turn output cap on the rollout
    (TokenRollout.generation_cap, e.g. NemoGym's max_output_tokens_per_step
    riding every /run body) — a generation of exactly cap tokens. The cap
    case is verified arithmetically from the generation mask rather than
    trusted from a label, so silent wire/mask truncation still fails the
    check. Uncapped rollouts keep the strict eod-or-seq_len contract.
    """
        ...
        for rollout in group:
            if isinstance(rollout, TokenRollout):
                for turn_traj in rollout.trajectory:
                    ...
                    # A single-turn completion can only end in eod, be truncated at
                    # seq_len, or stop exactly at the agent's stamped output cap.
                    ...
                    if len(rollout.trajectory) == 1:
                        assert single_turn_termination_ok(
                            ...
                            f"Single-turn rollout under seq_length must end in eod or "
                            f"exactly at its generation cap: env={rollout.env_id} "
                            f"len={len(turn_traj)} "
...
def _bounded_artifact_key(key, limit=100):
    ...
    let a 128-char ValueError kill f0d3 on all three links (fork, 2026-07-26).
    ...
    (port of fork 5ef736978cef3a3a5a82ddb3e2d33aebe2607f09, wandb hunks only)
    """
    if len(key) <= limit:
        return key
...
def prep_wandb_metrics(
    ...
    def _dist(prefix, values, title, native_hist=True):
        ...
        out = {
            ...
            _bounded_artifact_key(f'{prefix}_hist'): wandb_writer.plot.histogram(
...
def maybe_log_training_metrics(
    ...
    for env_id in set(group_stats.env_ids):
        ...
        for k, v in env_metrics.items():
            # Bound the key AFTER the env prefix, not before: inner helpers only
            # ever see the short local key (e.g. 'staleness/kv_cache/first_hist');
            # the long env_id prepended right here is what actually blows wandb's
            # 128-char artifact-name limit. On the fork, bounding early never
            # fired and f0d3 rank 63 raised the artifact ValueError at log time
            # on all three links — one rank exit(1)s, KillOnBadExit tears down
            # the other 63, and their crash files all read as cudaErrorContained
            # (a fake NVLink fault). Only Table/plot-backed values become
            # artifacts; plain scalars have no length limit, so those keys stay
            # fully readable.
