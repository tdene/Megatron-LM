    ...
    <bank_dir>/
        ...
        gen-<iter>/
            ledger.log                # append-only JSONL: one self-describing index
                                      ...
                                      # plus one "records" amendment per token group
                                      # carrying its finished-request ledger records
...
Finished-request records: per-rollout staleness metadata (policy/kv-cache epochs,
evictions) does not ride the rollouts on this branch; it lives in an engine-drained
request ledger keyed by the SHA-256 content of each turn's (prompt, generated)
token streams, joined at training consumption by ``rl_utils._pop_request_records``
— which hard-asserts on a miss. A banked group restored after a process restart
would therefore be untrainable without its records. Since the engine drain is a
collective that only happens at the end of a collection window, the records
cannot be written together with the group at assembly time. Instead ``append``
remembers the group's per-turn ledger keys, and once the drain lands trainer-side,
``attach_pending_request_records`` COPIES the group's records out of the runtime
ledger (never popping them — the live flow still pops each record exactly once at
consumption) into a ``kind="records"`` amendment line. ``restore`` re-attaches the
amendment to the group (``group.request_records``) so the caller can merge the
records back into the runtime ledger before the group is consumed; token groups
whose amendment never landed (killed before the drain) are dropped — their records
died with the engine, so they are regenerated instead.
...
import logging
import os
from typing import Iterator, Literal, NamedTuple, Optional, TypedDict
...
def rollout_request_keys(rollout: TokenRollout) -> list[tuple[bytes, bytes]]:
    """Per-turn content keys of a rollout in the finished-request ledger.

    Mirrors the engine-side keying in ``FinishedRequestRecord.from_request``: each
    turn keys by the SHA-256 digests of its prompt and generated token streams
    (int64 little-endian bytes), with the prompt/generation split recovered from
    the turn's generation-mask False-prefix. This is the single source of the key
    math for both the live ledger join (``rl_utils._pop_request_records``) and the
    durable rollout bank, so the two can never drift apart.

    Args:
        rollout: A TokenRollout whose per-turn keys to compute.

    Returns:
        One ``(prompt_digest, generated_digest)`` key per trajectory turn.
    """
    ...
    for tokens, mask in zip(rollout.trajectory, rollout.generation_mask):
        ...
        keys.append(
            (
                hashlib.sha256(
                    np.asarray(tokens[:prompt_length], dtype=np.int64).tobytes()
                ).digest(),
                hashlib.sha256(
                    np.asarray(tokens[prompt_length:], dtype=np.int64).tobytes()
                ).digest(),
...
class LedgerRecord(TypedDict):
    ...
    Args:
        ...
        kind: How the group is stored. "inline" if the group is not token-typed, "token" if the
            group is token-typed. A "records" line is not a group: it amends an earlier group
            record with the group's finished-request ledger records.
        ...
        request_records: Per-member, per-turn finished-request records ("records" kind only).
            ``None`` entries mark members that need no records (non-token rollouts).
    ...
    uid: str
    collection_iter: int
    member_type: NotRequired[Literal["Rollout", "TokenRollout"]]
    kind: Literal["inline", "token", "records"]
    group: NotRequired[dict]
    tok: NotRequired[SidecarMeta]
    lp: NotRequired[SidecarMeta]
    mask: NotRequired[SidecarMeta]
    request_records: NotRequired[list]
...
def _group_needs_request_records(group: RolloutGroup) -> bool:
    """True when consuming ``group`` will join the finished-request ledger.

    ``rl_utils._pop_request_records`` pops one ledger record per trajectory turn
    of every TokenRollout member (and hard-asserts on a miss), so any group with
    at least one such turn is only restorable together with its records.
    """
    return any(
        isinstance(rollout, TokenRollout) and len(rollout.trajectory) > 0
...
class RolloutBank:
    ...
    def __init__(self, bank_dir: str, *, max_bytes: int = 0) -> None:
        ...
        # Groups appended but whose finished-request records have not been drained
        # from the engines yet: (uid, per-member per-turn ledger keys). Flushed by
        # attach_pending_request_records at the post-drain merge point.
        self._pending_record_attach: list[tuple[str, list[Optional[list]]]] = []
    ...
    # ---------------------------------------------------------------- append
    def append(
        self, group: "RolloutGroup", uid: str | None = None, request_records: list | None = None
    ) -> str:
        ...
        Args:
            group: The completed group to persist.
            uid: Preserve an existing uid (compaction rewrite) instead of
                assigning a fresh one.
            request_records: The group's already-extracted finished-request
                records (per member, per turn), written immediately as a
                "records" amendment. When None (the live stage_filter path),
                the group's ledger keys are remembered instead and the records
                are written later by ``attach_pending_request_records`` — the
                engine has not drained them yet at append time.
        """
        ...
        if request_records is not None:
            self._append_request_records(uid, request_records)
        else:
            keys_per_member = self._request_record_keys(group)
            if keys_per_member is not None:
                self._pending_record_attach.append((uid, keys_per_member))

        self._bytes_written += len(tok_bytes) + len(lp_bytes) + len(mask_bytes)
        self._maybe_warn_over_cap()
        return uid

    def _request_record_keys(self, group: "RolloutGroup") -> Optional[list]:
        """Per-member per-turn ledger keys, or None when the group needs no records.

        Returns None (and logs) when a member's keys cannot be derived — the live
        ledger join would fail for such a group anyway, and banking must never
        take generation down with it.
        """
        keys_per_member: list[Optional[list]] = []
        needs_records = False
        for rollout in group.rollouts:
            if not isinstance(rollout, TokenRollout):
                keys_per_member.append(None)
                continue
            try:
                keys = rollout_request_keys(rollout)
            except (AssertionError, TypeError) as exc:
                logger.warning(
                    "RolloutBank could not derive request-ledger keys for a banked rollout "
                    "(%s); the banked copy of this group will not be restorable.",
                    exc,
                )
                return None
            keys_per_member.append(keys)
            needs_records = needs_records or bool(keys)
        return keys_per_member if needs_records else None

    def attach_pending_request_records(self, request_ledger: dict) -> int:
        """Persist the finished-request records of groups banked since the last drain.

        Called at the post-drain merge point — the first moment a banked group's
        records exist trainer-side and the last moment before consumption pops
        them. Records are COPIED (looked up, never popped): the exact-ledger
        doctrine is that every record is popped exactly once, by
        compute_group_stats, at training consumption; the on-disk copy only
        matters after a restart, when the live instances are gone.

        Args:
            request_ledger: The runtime ledger (persistent-merged with this
                collection's drain) to copy records from.

        Returns:
            The number of groups whose records were attached.
        """
        pending, self._pending_record_attach = self._pending_record_attach, []
        attached = 0
        for uid, keys_per_member in pending:
            request_records: list = []
            cursor: dict = {}
            missing = False
            for keys in keys_per_member:
                if keys is None:
                    request_records.append(None)
                    continue
                member_records = []
                for key in keys:
                    ...
                    if index >= len(bucket):
                        # More same-content turns in this group than drained records
                        # under the key. Same-key records are indistinguishable by
                        # content, so reuse the last one for the banked copy; the
                        # live join's own assert remains the authority on genuine
                        # pipeline corruption.
                        logger.warning(
                            "RolloutBank: group %s has more same-content turns than "
                            "ledger records under one key; reusing the last record "
                            "for its banked copy.",
                            uid,
                        )
                        index = len(bucket) - 1
                    member_records.append(dataclasses.asdict(bucket[index]))
                    cursor[key] = index + 1
                if missing:
                    break
                request_records.append(member_records)
            if missing:
                logger.warning(
                    "RolloutBank: no finished-request record found for a turn of banked "
                    "group %s; its banked copy will not be restorable.",
                    uid,
                )
                continue
            self._append_request_records(uid, request_records)
    ...
    def _append_request_records(self, uid: str, request_records: list) -> None:
        """Write one "records" amendment line for ``uid``."""
        assert self._seg_dir is not None, "set_collection() must be called before append()"
        record: LedgerRecord = {
            "format_version": _FORMAT_VERSION,
            "uid": uid,
    ...
    def _encode(self, group: "RolloutGroup", uid: str) -> EncodedGroup:
        ...
        dumped.pop("request_records", None)  # records live in their own amendment line
    ...
    def restore(self, trained_through: int) -> list["RolloutGroup"]:
        ...
        Restored groups carry their persisted finished-request records as
        ``group.request_records`` (per member, per turn); the caller must merge
        them back into the runtime request ledger before the group is consumed.
        Token groups whose records amendment never landed (the process died
        before the engine drain) are dropped: their records died with the
        engine, so they cannot be joined and are regenerated instead.
        """
        self._close_handles()  # flush any active writer before reading
        manifest = self._read_manifest()
        markers = self._read_markers(self.bank_dir)
        group_records: list[tuple[str, LedgerRecord]] = []
        # uid -> request_records, matched across segments (an amendment may land
        # in a later segment than its group when collections roll in between).
        ...
        for seg in manifest["segments"]:
            ...
            for record in self._read_ledger(seg_dir):
                if record.get("kind") == "records":
                    expected = _checksum({k: v for k, v in record.items() if k != "checksum"})
                    if expected != record.get("checksum"):
                        logger.debug(f"Checksum mismatch — dropping records amendment: {record}")
                        continue
                    amendments[record["uid"]] = record["request_records"]
                    continue
                group_records.append((seg_dir, record))
        restored: list["RolloutGroup"] = []
        for seg_dir, record in group_records:
            uid = record["uid"]
            ...
            if group is None:
                continue
            request_records = amendments.get(uid)
            if request_records is None and _group_needs_request_records(group):
                logger.warning(
                    "RolloutBank: banked group %s has no persisted finished-request "
                    "records (killed before the engine drain?); dropping it — it will "
                    "be regenerated.",
                    uid,
                )
                continue
            if request_records is not None:
                group.request_records = request_records
    ...
    def checkpoint(self, iteration: int) -> None:
        ...
        """
        self._close_handles()
        if self._pending_record_attach:
            # Should not happen: compaction runs at checkpoint boundaries, after the
            # collection's post-drain attach. A survivor still pending here would be
            # dropped by the restore() below (no records amendment yet); its later
            # amendment becomes an ignored orphan.
            logger.warning(
                "RolloutBank: compacting with %d group(s) still awaiting their "
                "finished-request records; those groups will not carry forward.",
                len(self._pending_record_attach),
            )
    ...
    def _rewrite_segment(self, seg_dir: str, iteration: int, groups: list["RolloutGroup"]) -> None:
        ...
        try:
            for group in groups:
                # Reuses the write-through encoder + fsync; restored groups carry
                # their records, so the amendment is rewritten alongside the group.
                self.append(
                    group,
                    uid=group.uid,
                    request_records=getattr(group, "request_records", None),
                )
        finally:
            self._close_handles()
